...
function toggleControls(disabled) {
    const radioButtons = document.querySelectorAll('input[name="technique"]');
    radioButtons.forEach(rb => rb.disabled = disabled);
    
    const lift25 = document.getElementById('lift25Btn');
    const lift50 = document.getElementById('lift50Btn');
    
    if (lift25) lift25.disabled = disabled;
    if (lift50) lift50.disabled = disabled;
    
    // Add visual feedback for disabled state
    const techArea = radioButtons[0].closest('div');
    if (techArea) techArea.style.opacity = disabled ? '0.5' : '1';
    if (lift25) lift25.parentElement.style.opacity = disabled ? '0.5' : '1';
}
...
function drawSkeleton() {
    ...
    if (liftState === 'DOWN') {
        ...
        currentPose = { ...poseStand }; // Ensure fresh copy
        currentPose.hand = { x: 40, y: -130 };
        currentPose.shoulder = { ...currentPose.shoulder, x: currentPose.shoulder.x + 10 };
    }
